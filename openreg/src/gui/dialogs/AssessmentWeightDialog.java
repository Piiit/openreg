...
public class AssessmentWeightDialog extends GuiDialog {
	...
	public AssessmentWeightDialog(Shell parent) {
		super(parent);
	}
	...
	private void createContents() {
		...
		shlDialog.setText("Modify an assessment weight");
	...
	public void loadData(Object mainId, Object subId) throws Exception {
		ArrayList<Row> ab = WeightedAssessmentQuery.getDataset(mainId, subId);
		if(ab.size() == 0) {
	...
	public void update() {
		try {
			if (loadedData != null){
				text.setText(loadedData.getValueAsStringNotNull("weight"));
			}
		} catch (Exception e) {
			e.printStackTrace();
			GuiTools.showMessageBox(shlDialog, e.getMessage());
