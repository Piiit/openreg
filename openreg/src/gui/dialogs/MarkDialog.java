...
public class MarkDialog extends GuiDialog {
	...
	private Text textCP;
	private Long currentMarkTypeId;
	...
	private void createContents() {
		...
		lblDescription.setText("Mark Representation *");
	...
	public void loadData(Object data) throws Exception {
		ArrayList<Row> ab = MarkQuery.getDataset(data);
		if(ab.size() == 0) {
			throw new Exception("No mark with ID " + data.toString() + " found.");
		}
		loadedData = ab.get(0); 
		currentMarkTypeId = loadedData.getValueAsLong("mark_type_id");
	}
	
	public void setMarkType(Long id) {
		currentMarkTypeId = id;
	}
	...
	public void store() {
		try {
			Row course = new Row();
			course.setValue("representation", GuiTools.nullIfEmptyTrimmed(text.getText()));
			String cp = GuiTools.nullIfEmptyTrimmed(textCP.getText());
			course.setValue("bound", (cp == null ? null : Float.parseFloat(cp)));
			course.setValue("mark_type_id", currentMarkTypeId);
			
			if(loadedData == null) {
				MarkQuery.insert(course);	
			} else {
				MarkQuery.update(loadedData.getValueAsLong("id"), course);
			}
	...
	public void update() {
		try {
			if (loadedData != null){
				text.setText(loadedData.getValueAsStringNotNull("representation"));
				textCP.setText(loadedData.getValueAsStringNotNull("bound"));
				shlDialog.setText("Modify a mark");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
