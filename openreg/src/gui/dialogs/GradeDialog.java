...
public class GradeDialog extends GuiDialog {
	...
	private void createContents() {
		...
		shlDialog.setText("Grade an assessments");
	...
	public void loadData(Object data) throws Exception {
		ArrayList<Row> query = GradeQuery.getDataset(data);
		if(query.size() == 0) {
			throw new Exception("No student with ID " + data.toString() + " found.");
		}
		loadedData = query.get(0); 
	}
	...
	public void update() {
		try {
			...
			if (loadedData != null){
				updateNameField(loadedData.getValueAsLong("student_id"));
				updateMainAssessmentField(loadedData.getValueAsLong("assessment_id"));
				updateSubAssessmentTable();
			} else {
	...
	private void updateSubAssessmentTable() {
		...
		try {
			for(Row row: GradeQuery.getSubAssessments(assessmentId, studentId)) {
				TableItem tableItem = new TableItem(table, SWT.NONE);
				tableItem.setData(row.getValueAsLong("student_id"));
	...
	private void updateMainAssessmentField(Long id) {
		try {
			combo.removeAll();
			int i = 0;
			for(Row row : GradeQuery.getFullDataset1(GuiTools.getIdFromCombo(comboName))) {
