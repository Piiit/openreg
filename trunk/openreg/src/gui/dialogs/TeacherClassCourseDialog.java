...
public class TeacherClassCourseDialog extends GuiDialog {
	...
	private void createContents() {
		shlAssignTeacher = new Shell(getParent(), SWT.DIALOG_TRIM
	...
	private void createClassCombo() {
		if (positionCourseComboY >= positionClassComboY) {
			...
			if (posY != DEFAULT_POSITION_Y) {
				posY = positionCourseComboY < positionClassComboY ? positionClassComboY
						: positionCourseComboY;
				lblSeparator.add(new Label(shlAssignTeacher, SWT.SEPARATOR
						| SWT.HORIZONTAL));
				int lengthX = SIZE_COMBO_X * 2 + COMBO_OFFSET_X + 10;
				lblSeparator.get(lblSeparator.size() - 1)
						.setBounds(STARTINGPOINT_UPPER_LEFT_CORNER - 5,
								positionCourseComboY - (COMBO_OFFSET_Y / 2),
								lengthX, 2);
			}

			combosClasses.add(new Combo(shlAssignTeacher, SWT.READ_ONLY));
			combosClasses.get(combosClasses.size() - 1).addSelectionListener(
			...
			combosClasses.get(combosClasses.size() - 1).setBounds(
					STARTINGPOINT_UPPER_LEFT_CORNER, posY, SIZE_COMBO_X,
					SIZE_COMBO_Y);

			linkAddClass.setBounds(STARTINGPOINT_UPPER_LEFT_CORNER
					+ COMBO_OFFSET_X + LINK_OFFSET_X, posY + SIZE_COMBO_Y
					+ LINK_OFFSET_Y, 81, 15);
	...
	private void createCourseCombo() {
		int posX = STARTINGPOINT_UPPER_LEFT_CORNER + SIZE_COMBO_X
				+ COMBO_OFFSET_X;

		combosCourses.add(new Combo(shlAssignTeacher, SWT.READ_ONLY));
		combosCourses.get(combosCourses.size() - 1).addSelectionListener(
				new SelectionAdapter() {
					@Override
					public void widgetSelected(SelectionEvent arg0) {
						int i = 0;
		...
		combosCourses.get(combosCourses.size() - 1).setToolTipText(
				(combosClasses.size() - 1) + "");
		combosCourses.get(combosCourses.size() - 1).setBounds(posX,
				positionCourseComboY, SIZE_COMBO_X, SIZE_COMBO_Y);

		linkAddCourse.setBounds(posX + LINK_OFFSET_X, positionCourseComboY
				+ SIZE_COMBO_Y + LINK_OFFSET_Y, 81, 15);

		positionCourseComboY = positionCourseComboY + SIZE_COMBO_Y
				+ COMBO_OFFSET_Y;
		update();
	}
	...
	private void createRemoveLinks() {
		removeRemoveLinks();
		int posX = STARTINGPOINT_UPPER_LEFT_CORNER + SIZE_COMBO_X * 2
				+ COMBO_OFFSET_X;
		for (int i = 0; i < combosCourses.size(); i++) {
			linksRemove.add(new Link(shlAssignTeacher, SWT.NONE));
			linksRemove.get(linksRemove.size() - 1).addSelectionListener(
					new SelectionAdapter() {
						@Override
						public void widgetSelected(SelectionEvent arg0) {
							int i = 0;
			...
			linksRemove.get(linksRemove.size() - 1).setBounds(
					posX + LINK_OFFSET_X, positionLinksRemoveY, 81, 15);

			positionLinksRemoveY = positionLinksRemoveY + SIZE_COMBO_Y
					+ COMBO_OFFSET_Y;
		}
	}
	...
	private void remove(int i) {
		...
		if (selection.length == 1) {
			...
			try {
				classId = Long
						.parseLong(combosClasses
								.get(Integer.parseInt(combo.getToolTipText()))
								.getData(
										combosClasses
												.get(Integer.parseInt(combo
														.getToolTipText()))
												.getItem(
														combosClasses
																.get(Integer
	...
	public void store(int comboClassIndex) throws Exception {
		String[] selection = listTeachers.getSelection();
		if (selection.length >= 1) {
			long teacherId = Long.parseLong(listTeachers.getData(selection[0])
					.toString());
			Combo combo = combosClasses.get(comboClassIndex);
			long classId = Long.parseLong(combo.getData(
					combo.getItem(combo.getSelectionIndex())).toString());
			for (int i = 0; i < combosCourses.size(); i++) {
				Combo course = combosCourses.get(i);
				long courseId;
				try {
					courseId = Long.parseLong(course.getData(
							course.getItem(course.getSelectionIndex()))
							.toString());
				} catch (Exception e) {
					Log.error("No course selected yet!");
					return;
				}
				if (Integer.parseInt(course.getToolTipText()) == comboClassIndex) {
					insertOrUpdateTuples(teacherId, classId, courseId, i);
				}
			}
	...
	private void insertOrUpdateTuples(long teacherId, long classId,
		...
		if (!updateTuple) {
			try {
				TeacherClassCourseQuery
						.insert(newTeacherClassCourse);
			} catch (Exception e) {
...
//			}
	...
	private void updateCombos() {
		for (Combo combo : combosClasses) {
			...
			for (Row c : loadedClasses) {
				String s = c.getValueAsString("level")
						+ c.getValueAsString("stream");
				combo.add(s);
	...
	public void setData() {
		if (!dataSet && loadedTeacherClassCourse != null
				&& loadedTeacherClassCourse.size() != 0) {
			setListData();
			setComboData();
			dataSet = true;
		}
	...
	private void setListData() {
		String name = loadedTeacherClassCourse.get(0).getValueAsString(
				"teacher_name")
				+ " "
	...
	private void setComboData() {
		...
		int id = 0;
		long teacher_id = loadedTeacherClassCourse.get(id).getValueAsLong(
				"teacher_id");
		long class_id = loadedTeacherClassCourse.get(id).getValueAsLong(
				"class_id");
		long course_id = loadedTeacherClassCourse.get(id).getValueAsLong(
				"course_id");

		combosClasses.get(class_counter).select(
				getComboClassIndex(combosClasses, class_id, class_counter));
		combosCourses.get(course_counter).select(
				getComboClassIndex(combosCourses, course_id, class_counter));
		class_counter += 1;
		course_counter += 1;
		...
		for (int i = 0; i < loadedTeacherClassCourse.size(); i++) {
			...
			if (class_id != ltcc.getValueAsLong("class_id")) {
				...
				combosClasses.get(class_counter).select(
						getComboClassIndex(combosClasses, class_id,
								class_counter));
				combosCourses.get(course_counter).select(
						getComboClassIndex(combosCourses, course_id,
								class_counter));
				class_counter += 1;
				course_counter += 1;
			...
			Log.info("Inserted the following tuples: " + teacher_id + " "
					+ class_id + " " + course_id);
		}
	}
