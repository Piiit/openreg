...
public class AssessmentTypeDialog extends GuiDialog {
	...
	public AssessmentTypeDialog(Shell parent) {
		super(parent);
	}
	...
	private void createContents() {
		...
		shlDialog.setText("Add a new assessment type");
		...
		lblDescription.setText("Assessment Type *");
	...
	public void loadData(Object data) throws Exception {
		ArrayList<Row> ab = AssessmentTypeQuery.getDataset(data);
		if(ab.size() == 0) {
			throw new Exception("No assessment type with ID " + data.toString() + " found.");
		}
		loadedData = ab.get(0); 
	}
	...
	public void store() {
		try {
			...
			if(loadedData == null) {
				AssessmentTypeQuery.insert(newDescription);	
			} else {
				AssessmentTypeQuery.update(loadedData.getValueAsLong("id"), newDescription);
			}
	...
	public void update() {
		try {
			if (loadedData != null){
				...
				shlDialog.setText("Modify the assessment type");
			}
		} catch (Exception e) {
			e.printStackTrace();
			GuiTools.showMessageBox(shlDialog, e.getMessage());
